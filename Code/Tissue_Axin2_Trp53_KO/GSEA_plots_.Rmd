...
```{r}
selected_genesets = c("Lgr5_SC_Signature_Munoz", "Herbst_et_al_Beta_catenin_targets", "Yui_DSS_colitis_vs_control_down", "Yui_DSS_colitis_vs_control_up", "Gregorieff_Yap_KO_up", "Gregorieff_Yap_KO_down", "Gregorieff_Yap_Tg_up", "Gregorieff_Yap_Tg_down")

selected_genesets = append(selected_genesets, strsplit("WNT_SIGNALING
...
KLEEMAN_Global.Wnt.targets
KLEEMAN_Intestinal.stem.cell.Wnt.targets
KLEEMAN_Proliferative.cell.Wnt.target.genes
KLEEMAN_Wnt.negative.regulator.genes
Boulter_Wnt_genes","\n") |> unlist())
```
...
for (sel_comparison in names(all_results)) {
  ...
  for (g in selected_genesets) {
    ...
    if(length(loc)>0) {
      ...
      if(length(loc) == 0) {warning(paste0("Did not find results for gene set ",g,". Skipping.")); next;}
