...
all_ge_data_filtered <- readRDS(file.path(data_storage_folder, "GSE201723.combined.rds"))
DefaultAssay(all_ge_data_filtered) <- "RNA"
all_ge_data_filtered <- DietSeurat(all_ge_data_filtered, assays = "RNA")
all_ge_data_filtered <- JoinLayers(all_ge_data_filtered)
sel_conditions <- c("d5_24h_aGFP","d5_24h_Uninj","d6_48h_aGFP","d6_48h_Uninj")
all_ge_data_filtered <- subset(all_ge_data_filtered, seurat_clusters %in% c(2,3,4,6,7,8,16,22) & (is.na(cell_type) | cell_type=="epithelium") & condition %in% sel_conditions)
...
```{r}
reference_dataset <- which(names(sc.list) %in% c("d5_24h_Uninj_1","d5_24h_Uninj_2", "d6_48h_Uninj_1","d6_48h_Uninj_2")) 
...
scData.combined <- RunUMAP(scData.combined, reduction = "pca", dims = 1:15)
scData.combined <- FindNeighbors(scData.combined, reduction = "pca", dims = 1:15)
scData.combined <- FindClusters(scData.combined, resolution = 0.3)
```

```{r}
saveRDS(scData.combined, file=file.path(data_storage_folder, "GSE201723_epi_only.combined_no_SWAP.rds"), compress=T)
#scData.combined <- readRDS(file.path(data_storage_folder, "GSE201723_epi_only.combined.rds"))
```
